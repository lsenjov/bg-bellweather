...
<p class="intro">Thirty unique policies: every ordered pair of six promises appears once. Each card has a + category, a − category, six printed party votes, and an ongoing effect. <a href="printable-policy-cards.html">Print the 30-card deck</a>.</p>
...
<section><h2>Law resolution</h2><p>Laws modifying an Operation also modify Bonus cards that explicitly resolve that Operation. Standalone movement or placement is governed by its own card. Each distinct law effect applies at most once per card action, even if two enacted policies print it or two Support move. An Operate turn may resolve several separate card actions.</p><p>Combine applicable movement permissions: a law may change where Organise can go while Political Exchange permits a swap. With Carpooling, each of two arriving Support needs a free spot or a rival swapped back to the source. Choose the rivals; both arrivals use the same source and destination.</p><p>Resolve the baseline Operation, then its triggered law follow-ups in the acting firm’s chosen order. Extra placements and movements are mandatory when possible and otherwise skipped; an unavailable law extra does not block the original Operation. Text offering an alternative remains a choice.</p><p>Follow-ups trigger only from the original Operation, never from other follow-ups. Bridge Campaign requires a direct move across a marked bridge; a possible route for a long-distance move is insufficient. Cross-Region Campaigns compares the original source and destination, including Centre. Carpooling does not double extras. No loops.</p><p>Identical effects do not stack. Different effects combine. All normal capacity limits and bridge-defined adjacency still apply unless a law explicitly changes the relevant movement or targeting permission.</p></section>
...
<section><h2>Open interaction questions</h2><ul><li>When law follow-ups and a Bonus card’s own additional effect compete for the same space, can the firm interleave them or must one group resolve first?</li><li>How should Displacement interact with Smear Bonuses that refer to removed Support or a vacated spot (especially Spin, Stonewall and Midnight Leak)?</li><li>How should Political Exchange interact with Dig In when the source spot is occupied by a swapped rival?</li></ul><p>These cases need a ruling before using the affected combination; no additional rule is assumed here.</p></section>`));
...
output('docs/components/printable-policy-cards.html', printPage('Policy deck', 'Thirty 63 × 88 mm cards on four sheets. Deal one face up per region. Use identical opaque sleeves for the draw deck.', 'policy-sheet', policies.map((c) => `<article class="policy-card"><header><b>Policy proposal</b><span>${c.id}</span></header><h2>${escape(c.name)}</h2><div class="promises"><p><b>+</b> ${c.plus}</p><p><b>−</b> ${c.minus}</p></div><div class="votes"><p><b>FOR</b>${votes(c, true).map((p) => `<span>${p.name}</span>`).join('')}</p><p><b>AGAINST</b>${votes(c, false).map((p) => `<span>${p.name}</span>`).join('')}</p></div><div class="law"><h3>${effectFor(c).name}</h3><p>${effectFor(c).text}</p></div><footer>Pass: For &gt; Against · Global law<br>Each distinct effect once per card action</footer></article>`), 9));
