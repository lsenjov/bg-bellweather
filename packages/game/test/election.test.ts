...
import { POLICIES, POLICIES_BY_ID, REGION_IDS, SCORING_CARDS, PARTY_IDS, votesFor, type PolicyId } from "@bellweather/content";
...
describe("policy elections",()=>{
  ...
  it("thins every non-Centre district and never draws Centre",()=>{
    ...
    expect(Object.keys(draws)).toHaveLength(15); expect(draws['bellweather-centre']).toBeUndefined();
    expect(draws.harbormouth!.parties).toHaveLength(3); expect(state.support['bellweather-centre']).toEqual({honeycomb:3});
  ...
  it("counts tokens rather than parties, passes strict majorities and fails empty/tied regions",()=>{
    ...
    state.support['bellweather-centre']={[yes]:2,[no]:1};
    ...
    input.state.districts['bellweather-centre']!.support={[yes]:1,[no]:1};
  ...
  it("activates scoring laws passed in the final vote before calculating points",()=>{
    ...
    state.pendingPolicies.centre=muted.id;state.support['bellweather-centre']={[PARTY_IDS.find(p=>votesFor(p,muted))!]:3};
