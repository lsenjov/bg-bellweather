...
2. **Engine and protocol.** Replace once-per-seat opening completion with an ordered repeated-seat sequence and cursor, accept exactly one affordable opening or an insolvent pass per turn, preserve replay determinism, advance to ruleset version 9, and cover every Early Bird rotation, wraparound, turn-around, insolvency, gifts, and standard-player order.
